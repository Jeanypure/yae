...
class MinisterAgreestSearch extends PurInfo
{
    ...
    {
        return [
            [['is_purchase','is_agreest','is_quality','pur_info_id', 'pur_group', 'is_huge', 'pd_purchase_num', 'has_shipping_fee', 'bill_tax_rebate', 'parent_product_id', 'source', 'preview_status', 'brocast_status', 'master_result', 'is_submit', 'is_submit_manager', 'pur_group_status', 'junior_submit', 'is_assign', 'audit_a', 'audit_b', 'bill_tax_value', 'pur_complete_status', 'pur_compelte_result', 'sample_submit2', 'sample_submit1'], 'integer'],
        ...
        if($username=='Jenny'||$username=='David'||$username=='Mark'){
            $query = PurInfo::find()
                ->select(['
                    ...
                    `pur_info`.sample_submit2,`pur_info`.is_quality,`pur_info`.submit2_at,`pur_info`.is_purchase,  
            ...
            $query = PurInfo::find()
                ->select(['
                    ...
                    `pur_info`.pd_title,`pur_info`.pd_title_en,`pur_info`.purchaser,`pur_info`.pd_pic_url,
                    `pur_info`.pur_group,`pur_info`.master_result,`pur_info`.master_mark,
                    `pur_info`.sample_submit1,`pur_info`.is_quality,`pur_info`.submit1_at,`pur_info`.is_purchase,  
                    `sample`.spur_info_id'])
                ->joinWith('sample')
