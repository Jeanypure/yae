...
class MinisterAgreestSearch extends PurInfo
{
    ...
    public function rules()
    {
        return [
            ...
            [['pd_sku','sure_purchase_time','write_date','submit2_at','purchaser', 'pd_title', 'pd_title_en', 'pd_pic_url', 'pd_package', 'pd_length', 'pd_width', 'pd_height', 'pd_material', 'bill_type', 'hs_code', 'bill_rebate_amount', 'no_rebate_amount', 'retail_price', 'ebay_url', 'amazon_url', 'url_1688', 'else_url', 'shipping_fee', 'oversea_shipping_fee', 'transaction_fee', 'gross_profit', 'remark', 'member', 'master_member', 'master_mark', 'priview_time', 'pd_create_time', 'purchaser_leader', 'profit_rate', 'gross_profit_amz', 'profit_rate_amz', 'amz_fulfillment_cost', 'selling_on_amz_fee', 'amz_retail_price', 'amz_retail_price_rmb', 'commit_date'], 'safe'],
        ...
        if( array_key_exists('超级管理员',$role)){
            $query = PurInfo::find()
                ->select(['
                    ...
                    `pur_info`.submit1_at,`pur_info`.sample_return,`pur_info`.has_pay,`pur_info`.payer,`pur_info`.pay_at,`pur_info`.sure_purchase_time,  
            ...
            $query = PurInfo::find()
                ->select(['
                    ...
                    `pur_info`.pd_title,`pur_info`.pd_title_en,`pur_info`.purchaser,`pur_info`.pd_pic_url,
                    `pur_info`.pur_group,`pur_info`.master_result,`pur_info`.master_mark,
                    `pur_info`.sample_submit2,`pur_info`.is_quality,`pur_info`.submit1_at,`pur_info`.is_purchase,
                    `pur_info`.submit2_at,
                    `pur_info`.submit1_at,`pur_info`.sample_return,`pur_info`.has_pay,`pur_info`.payer,`pur_info`.pay_at,
                    `pur_info`.sure_purchase_time,   
                    `sample`.spur_info_id,`sample`.is_agreest,`sample`.for_free,`sample`.has_arrival,
                    `sample`.minister_result,`sample`.pd_sku,`sample`.write_date'])
                ->joinWith('sample')
