        ...
        if($username=='Jenny'||$username=='David'||$username=='Mark'){
            $query = PurInfo::find()
                ->select(['
                    ...
                    `pur_info`.payer,`pur_info`.has_pay,`pur_info`.pay_at,`pur_info`.submit2_at,
                    `sample`.spur_info_id,`sample`.create_date,`sample`.is_agreest'])
                ->joinWith('sample')
