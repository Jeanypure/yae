...
class PurInfoTrackSearch extends PurInfo
{
    ...
    public function rules()
    {
        return [
            ...
            [['has_pay','is_quality','spur_info_id','sample_submit1','pur_info_id', 'pur_group', 'is_huge', 'pd_purchase_num', 'has_shipping_fee', 'bill_tax_value', 'bill_tax_rebate', 'parent_product_id', 'source', 'preview_status', 'brocast_status', 'master_result', 'is_submit', 'is_submit_manager', 'pur_group_status', 'junior_submit', 'is_assign'], 'integer'],
        ...
        if($username=='Jenny'||$username=='David'||$username=='Mark'){
            $query = PurInfo::find()
                ->select(['
                    ...
                    `pur_info`.payer,`pur_info`.has_pay,`pur_info`.pay_at,
