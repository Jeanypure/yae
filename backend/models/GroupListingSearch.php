 ...
 */
class GroupListingSearch extends PurInfo
{
    /**
     * {@inheritdoc}
     */
    ...
    public function search($params)
    {
        ...
        $res = Purchaser::find()->select('group_listing')
            ->where("purchaser='".$username."'")->asArray()->one();


        if($res['group_listing']==0){
            $query = PurInfo::find()
            ...
            $query = PurInfo::find()
                ->select(['`pur_info`.*,`sample`.pd_sku'])
                ->joinWith('sample')
                ->andWhere(['pur_group'=>$res['group_listing']])
                ->andWhere(['is_submit'=>1])
                ->andWhere(['is_purchase'=>1])
                ->orderBy('pur_info_id   desc');
        }
